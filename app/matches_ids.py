...
def get_matches_ids(
    ...
    match_ids_query = db.session.query(SoccerTimeline.match_id).filter(
    ...
    if country and country != 'None':
        match_ids_query = match_ids_query.join(
        ...
        match_ids_query = match_ids_query.distinct()

    if league and league != 'null':
        match_ids_query = match_ids_query.filter(
    ...
    # Применяем фильтрацию по xG, ударам, и другим статистическим данным
