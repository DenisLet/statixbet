...
def process_corners(
    ...
    if total_corners_entries == 0:
        return total_corners_entries, [], [], []
    ...
    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        corners_query_team1 = corners_query_team1.join(
            SoccerMain, SoccerTimeline.match_id == SoccerMain.match_id
        ).join(
            ChampionshipsSoccer, SoccerMain.league_id == ChampionshipsSoccer.id
        ).filter(
    ...
    if league and league != 'null':
        corners_query_team1 = corners_query_team1.filter(
            SoccerMain.league_id == league
        )
    ...
    team1_corners_entries = corners_query_team1.group_by(
        'team1_corners_h2'
    ).order_by(
        'team1_corners_h2'
    ).all()
    ...
    team1_corners_percentages = [
        (entry.team1_corners_h2, entry.count, (entry.count / total_team1_corners_entries) * 100)
    ...
    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        corners_query_team2 = corners_query_team2.join(
            SoccerMain, SoccerTimeline.match_id == SoccerMain.match_id
        ).join(
            ChampionshipsSoccer, SoccerMain.league_id == ChampionshipsSoccer.id
        ).filter(
    ...
    if league and league != 'null':
        corners_query_team2 = corners_query_team2.filter(
            SoccerMain.league_id == league
        )
    ...
    team2_corners_entries = corners_query_team2.group_by(
        'team2_corners_h2'
    ).order_by(
        'team2_corners_h2'
    ).all()
    ...
    return total_corners_entries, result_table, team1_corners_percentages, team2_corners_percentages