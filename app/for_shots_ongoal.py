...
def process_shots_ongoal(
    ...
    # Создаем алиас для SoccerMain
    SoccerMainAlias = aliased(SoccerMain)

    # Запрос для неудачных ударов
    ongoal_query = db.session.query(
        (SoccerHalf2Stats.home_shots + SoccerHalf2Stats.away_shots).label('total_ongoal_h2'),
        ...
        SoccerHalf2Stats.home_shots.between(0, 20),
        SoccerHalf2Stats.away_shots.between(0, 20),
        SoccerMainAlias.final != 'Awarded'  # Фильтруем по полю final через алиас
    )

    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        ongoal_query = ongoal_query.join(
    ...
    if league and league != 'null':
        ongoal_query = ongoal_query.filter(
    ...
    ongoal_entries = ongoal_query.group_by(
        'total_ongoal_h2'
    ).order_by(
        'total_ongoal_h2'
    ).all()

    total_ongoal_entries = sum(entry.count for entry in ongoal_entries)

    if total_ongoal_entries == 0:
        return total_ongoal_entries, [], [], []

    ongoal_percentages = [
        (entry.total_ongoal_h2, entry.count, (entry.count / total_ongoal_entries) * 100)
        for entry in ongoal_entries
    ]

    SoccerMainAlias = aliased(SoccerMain)

    # Запрос для команды 1
    ongoal_query_team1 = db.session.query(
        SoccerHalf2Stats.home_shots.label('team1_ongoal_h2'),
        func.count().label('count')
    ).join(
        SoccerTimeline, SoccerTimeline.match_id == SoccerHalf2Stats.match_id
    ).join(
    ...
    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        ongoal_query_team1 = ongoal_query_team1.join(
            SoccerMain, SoccerTimeline.match_id == SoccerMain.match_id
        ).join(
            ChampionshipsSoccer, SoccerMain.league_id == ChampionshipsSoccer.id
        ).filter(
    ...
    if league and league != 'null':
        ongoal_query_team1 = ongoal_query_team1.filter(
            SoccerMain.league_id == league
        )
    ...
    team1_ongoal_entries = ongoal_query_team1.group_by(
        'team1_ongoal_h2'
    ).order_by(
        'team1_ongoal_h2'
    ).all()
    ...
    total_team1_ongoal_entries = sum(entry.count for entry in team1_ongoal_entries)



    team1_ongoal_percentages = [
        (entry.team1_ongoal_h2, entry.count, (entry.count / total_team1_ongoal_entries) * 100)
        for entry in team1_ongoal_entries
    ]

    SoccerMainAlias = aliased(SoccerMain)

    # Запрос для команды 2
    ongoal_query_team2 = db.session.query(
        SoccerHalf2Stats.away_shots.label('team2_ongoal_h2'),
        func.count().label('count')
    ).join(
        SoccerTimeline, SoccerTimeline.match_id == SoccerHalf2Stats.match_id
    ).join(
    ...
    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        ongoal_query_team2 = ongoal_query_team2.join(
            SoccerMain, SoccerTimeline.match_id == SoccerMain.match_id
        ).join(
            ChampionshipsSoccer, SoccerMain.league_id == ChampionshipsSoccer.id
        ).filter(
    ...
    if league and league != 'null':
        ongoal_query_team2 = ongoal_query_team2.filter(
            SoccerMain.league_id == league
        )
    ...
    team2_ongoal_entries = ongoal_query_team2.group_by(
        'team2_ongoal_h2'
    ).order_by(
        'team2_ongoal_h2'
    ).all()
    ...
    total_team2_ongoal_entries = sum(entry.count for entry in team2_ongoal_entries)


    team2_ongoal_percentages = [
        (entry.team2_ongoal_h2, entry.count, (entry.count / total_team2_ongoal_entries) * 100)
        for entry in team2_ongoal_entries
    ]
    ...
    for i, (total_ongoal, count, percentage) in enumerate(ongoal_percentages):
        if i < len(ongoal_percentages) - 1:
            greater_percentage = 100 - cumulative_percentage - percentage
        else:
            greater_percentage = None

        result_table.append((total_ongoal, count, percentage, greater_percentage))
        cumulative_percentage += percentage


    return total_ongoal_entries, result_table, team1_ongoal_percentages, team2_ongoal_percentages