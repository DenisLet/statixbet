...
def process_offsides(
    ...
    if country and country != 'None':
        offsides_query = offsides_query.join(
    ...
    if league and league != 'null':
        offsides_query = offsides_query.filter(
    ...
    offsides_entries = offsides_query.group_by(
        'total_offsides_h2'
    ).order_by(
        'total_offsides_h2'
    ).all()

    total_offsides_entries = sum(entry.count for entry in offsides_entries)

    if total_offsides_entries == 0:
        return total_offsides_entries, [], [], []

    offsides_percentages = [
        (entry.total_offsides_h2, entry.count, (entry.count / total_offsides_entries) * 100)
    ...
    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        offsides_query_team1 = offsides_query_team1.join(
            SoccerMain, SoccerTimeline.match_id == SoccerMain.match_id
        ).join(
            ChampionshipsSoccer, SoccerMain.league_id == ChampionshipsSoccer.id
        ).filter(
    ...
    if league and league != 'null':
        offsides_query_team1 = offsides_query_team1.filter(
            SoccerMain.league_id == league
        )
    ...
    team1_offsides_entries = offsides_query_team1 .group_by(
        'team1_offsides_h2'
    ).order_by(
        'team1_offsides_h2'
    ).all()
    ...
    total_team1_offsides_entries = sum(entry.count for entry in team1_offsides_entries)



    team1_offsides_percentages = [
        (entry.team1_offsides_h2, entry.count, (entry.count / total_team1_offsides_entries) * 100)
    ...
    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        offsides_query_tean2 = offsides_query_tean2.join(
            SoccerMain, SoccerTimeline.match_id == SoccerMain.match_id
        ).join(
            ChampionshipsSoccer, SoccerMain.league_id == ChampionshipsSoccer.id
        ).filter(
    ...
    if league and league != 'null':
        offsides_query_tean2 = offsides_query_tean2.filter(
            SoccerMain.league_id == league
        )
    ...
    team2_offsides_entries = offsides_query_tean2.group_by(
        'team2_offsides_h2'
    ).order_by(
        'team2_offsides_h2'
    ).all()
    ...
    total_team2_offsides_entries = sum(entry.count for entry in team2_offsides_entries)


    team2_offsides_percentages = [
        (entry.team2_offsides_h2, entry.count, (entry.count / total_team2_offsides_entries) * 100)
        for entry in team2_offsides_entries
    ]
    ...
    for i, (total_offsides, count, percentage) in enumerate(offsides_percentages):
        if i < len(offsides_percentages) - 1:
            greater_percentage = 100 - cumulative_percentage - percentage
        else:
            greater_percentage = None

        result_table.append((total_offsides, count, percentage, greater_percentage))
        cumulative_percentage += percentage


    return total_offsides_entries, result_table, team1_offsides_percentages, team2_offsides_percentages