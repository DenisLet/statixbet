...
def process_fouls(
    ...
    # Запрос для фолов
    fouls_query = db.session.query(
        (SoccerHalf2Stats.home_fouls + SoccerHalf2Stats.away_fouls).label('total_fouls_h2'),
        ...
        SoccerHalf2Stats.home_fouls.between(0, 35),
        SoccerHalf2Stats.away_fouls.between(0, 35),
        SoccerMainAlias.final != 'Awarded'  # Фильтруем по полю final через алиас
    )

    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        fouls_query = fouls_query.join(
    ...
    if league and league != 'null':
        fouls_query = fouls_query.filter(
    ...
    fouls_entries = fouls_query.group_by(
        'total_fouls_h2'
    ).order_by(
        'total_fouls_h2'
    ).all()

    total_fouls_entries = sum(entry.count for entry in fouls_entries)

    if total_fouls_entries == 0:
        return total_fouls_entries, [], [], []

    fouls_percentages = [
        (entry.total_fouls_h2, entry.count, (entry.count / total_fouls_entries) * 100)
        for entry in fouls_entries
    ]

    SoccerMainAlias = aliased(SoccerMain)

    # Запрос для фолов команды 1
    fouls_query_team1 = db.session.query(
        SoccerHalf2Stats.home_fouls.label('team1_fouls_h2'),
        func.count().label('count')
    ).join(
        SoccerTimeline, SoccerTimeline.match_id == SoccerHalf2Stats.match_id
    ).join(
    ...
    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        fouls_query_team1 = fouls_query_team1.join(
            SoccerMain, SoccerTimeline.match_id == SoccerMain.match_id
        ).join(
            ChampionshipsSoccer, SoccerMain.league_id == ChampionshipsSoccer.id
        ).filter(
    ...
    if league and league != 'null':
        fouls_query_team1 = fouls_query_team1.filter(
            SoccerMain.league_id == league
        )
    ...
    team1_fouls_entries = fouls_query_team1.group_by(
        'team1_fouls_h2'
    ).order_by(
        'team1_fouls_h2'
    ).all()
    ...
    total_team1_fouls_entries = sum(entry.count for entry in team1_fouls_entries)



    team1_fouls_percentages = [
        (entry.team1_fouls_h2, entry.count, (entry.count / total_team1_fouls_entries) * 100)
        for entry in team1_fouls_entries
    ]

    SoccerMainAlias = aliased(SoccerMain)

    # Запрос для фолов команды 2
    fouls_query_team2 = db.session.query(
        SoccerHalf2Stats.away_fouls.label('team2_fouls_h2'),
        func.count().label('count')
    ).join(
        SoccerTimeline, SoccerTimeline.match_id == SoccerHalf2Stats.match_id
    ).join(
    ...
    # Фильтрация по странам, лигам и командам
    if country and country != 'None':
        fouls_query_team2 = fouls_query_team2.join(
            SoccerMain, SoccerTimeline.match_id == SoccerMain.match_id
        ).join(
            ChampionshipsSoccer, SoccerMain.league_id == ChampionshipsSoccer.id
        ).filter(
    ...
    if league and league != 'null':
        fouls_query_team2 = fouls_query_team2.filter(
            SoccerMain.league_id == league
        )
    ...
    team2_fouls_entries = fouls_query_team2.group_by(
        'team2_fouls_h2'
    ).order_by(
        'team2_fouls_h2'
    ).all()
    ...
    total_team2_fouls_entries = sum(entry.count for entry in team2_fouls_entries)


    team2_fouls_percentages = [
        (entry.team2_fouls_h2, entry.count, (entry.count / total_team2_fouls_entries) * 100)
        for entry in team2_fouls_entries
    ]
    ...
    for i, (total_fouls, count, percentage) in enumerate(fouls_percentages):
        if i < len(fouls_percentages) - 1:
            greater_percentage = 100 - cumulative_percentage - percentage
        else:
            greater_percentage = None

        result_table.append((total_fouls, count, percentage, greater_percentage))
        cumulative_percentage += percentage


    return total_fouls_entries, result_table, team1_fouls_percentages, team2_fouls_percentages